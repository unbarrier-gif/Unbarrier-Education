...
| 4 | `/voice` | orchid mist | built under the legal hold on the instrument (no delivered/tool split, no cohorts). Closes on "we agree how you will know it worked, and when we will check." Completed in stage 9. |
...
**Not carried over from the pack, on purpose.** `/resources` (the site plan's settled decision is that the free resources live on `/hello`; home's "everything free" ghost points there). The `/about` stub (`h0`…`h2`, "copy to come from nici") — the route already carries the approved 28 Aug copy. The `Neurodiversity Conversation` page (on the shelf as "url to confirm", not in the sequence).

| 9 | `/voice` | orchid mist | **the legal hold on the instrument is lifted, permanently (Nici, 14 Sep 2026).** v3 (delivered · the tool) and v6 (founding cohorts, the page's one `.pull`) are built from the approved 28 Aug copy. The route is indexed, in the sitemap, in the nav and in the footer. The `voicePublic` flag is deleted, not flipped: a lifted hold is not an open decision. Still true: no price for voice on its own, no date on the cohorts. The voice baseline paper (`docs/print/voice-baseline.html`) is unchanged — its copy was approved as canonical for voice on paper. |
...
`emailStep`, `shareCode` (readiness check), `handsMoveOnHello`.
